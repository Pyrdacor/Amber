    ...
    where T : unmanaged, IEquatable<T>
{
    ...
    {
        if (disposed)
            throw new AmberException(ExceptionScope.Render, "Tried to bind a disposed buffer.");
    ...
    internal bool RecreateUnbound()
    {
        ...
        if (disposed)
            throw new AmberException(ExceptionScope.Render, "Tried to recreate a disposed buffer.");
    ...
    protected static T[] EnsureBufferSize(T[] buffer, int size, out bool changed)
    {
        ...
        if (buffer == null)
        {
            ...
            // first we just use a buffer with 256 entries
            return new T[256];
        }
