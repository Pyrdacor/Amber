    ...
    internal static class PADP
    {
        ...
        public static Dictionary<ushort, T> Read<T>(IDataReader reader, GameData gameData) where T : IFileSpec, new()
        {
            ...
            if (readFileType != expectedFileType)
            {
                ...
                throw new AmberException(ExceptionScope.Data, $"{type} contains different content than expected. Requested {expectedFileType}, but found {readFileType}.");
            }
        ...
        private static Dictionary<ushort, IFileSpec> InternalRead(IDataReader reader, GameData gameData, ref bool wasPADF, byte? supportedVersion = null)
        {
            ...
            if (!FileHeader.CheckHeader(reader, Header, true))
                throw new AmberException(ExceptionScope.Data, "The file is no PADP");

            string fileType = reader.ReadString(3);

            if (!PADF.FileSpecs.TryGetValue(fileType, out var fileSpecProvider))
                throw new AmberException(ExceptionScope.Data, $"Unknown data files in PADP: {fileType}");
            ...
            if (fileSpecVersion > supportedVersion)
                throw new AmberException(ExceptionScope.Data, $"This application only supports {fileType} versions up to {(int)supportedVersion} but file has version {(int)fileSpecVersion}");

            var files = new Dictionary<ushort, IFileSpec>();
            int fileCount = reader.ReadWord();

            if (fileCount >= ushort.MaxValue)
                throw new AmberException(ExceptionScope.Data, $"Too many files given in PADP. Allowed are {ushort.MaxValue - 1}, given are {fileCount}.");
        ...
        public static void Write<T>(IDataWriter writer, IEnumerable<T> fileSpecs, ICompression? compression = null) where T : IFileSpec, new()
        {
            if (fileSpecs.Count() >= ushort.MaxValue)
                throw new AmberException(ExceptionScope.Data, $"Too many files given for PADP. Allowed are {ushort.MaxValue - 1}, given are {fileSpecs.Count()}.");
        ...
        private static void InternalWrite<T>(IDataWriter writer, IDictionary<ushort, T> fileSpecs, ICompression? compression, bool writeNoFileIndices) where T : IFileSpec, new()
        {
            ...
            if (!writeNoFileIndices && fileSpecs.Keys.Any(k => k == 0))
                throw new AmberException(ExceptionScope.Data, $"Sub-file key 0 is now allowed. Make sure to start at index 1.");
            ...
            foreach (var fileSpec in fileSpecs)
            {
                ...
                if (size > ushort.MaxValue)
                    throw new AmberException(ExceptionScope.Data, $"Sub-file {fileSpec.Key} is too large. Max size is {ushort.MaxValue}, but file had {size}.");
