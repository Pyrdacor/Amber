    ...
    public class GameData : IGameData, IGraphicProvider
    {
        ...
        public GameData(Stream stream)
        {
            ...
            if (!FileHeader.CheckHeader(reader, "PYGD", true))
                throw new AmberException(ExceptionScope.Data, "The given file is no Pyrdacor game data file.");
            ...
            places = new Lazy<Places>(() =>
            {
                ...
                if (locationData.Count != locationNames.Count)
                    throw new AmberException(ExceptionScope.Data, "Mismatch between number of location data and location name entries.");

                foreach (var location in locationData)
                {
                    if (!locationNames.TryGetValue(location.Key, out var name))
                        throw new AmberException(ExceptionScope.Data, $"Missing location name for location data {location.Key}.");
            ...
            for (int i = 0; i < fileCount; ++i)
            {
                ...
                if (!fileHandlers.TryGetValue(file, out var loader))
                    throw new AmberException(ExceptionScope.Data, $"No loader found for file '{file}' inside game data.");
