...
namespace Amber.GameData.Pyrdacor.FileSpecs
{
    internal class MapData : IFileSpec
    {
            ...
            else
            {
                ...
                if (usedEventCount != 0)
                {
                    if (map.EventList.Count == 0)
                        throw new AmberException(ExceptionScope.Data, "The map has no events but references some.");
                    ...
                    for (int i = 0; i < usedEventCount; ++i)
                    {
                        ...
                        if (tileIndex >= map.Width * map.Height)
                            throw new AmberException(ExceptionScope.Data, "Invalid tile index for map event.");

                        uint eventIndex = dataReader.ReadByte();

                        if (eventIndex >= map.EventList.Count)
                            throw new AmberException(ExceptionScope.Data, "Invalid event index for map event.");
                ...
                if (characterCount > 32)
                    throw new AmberException(ExceptionScope.Data, "Too many characters on map.");
        ...
        {
            if (map == null)
                throw new AmberException(ExceptionScope.Application, "Map data was null when trying to write it.");
