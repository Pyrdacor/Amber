...
namespace Amber.GameData.Pyrdacor.FileSpecs
{
    public class GraphicAtlas : IFileSpec
    {
        private static class TexturePacker
        {
            private class EmptyArea
            {
                public class Comparer : IComparer<EmptyArea>
                {
                    int IComparer<EmptyArea>.Compare(EmptyArea? x, EmptyArea? y)
                    {
                        ...
                        if (y is null)
                            return 1;
                        return (x.area.Size.Width * x.area.Size.Height).CompareTo(y.area.Size.Width * y.area.Size.Height);
                    }
                }
                ...
                public FillResult TryAddArea(Size areaSize, out Position? position)
                {
                    ...
                    if (areaSize.Width > area.Size.Width || areaSize.Height > area.Size.Height)
                    ...
                    if (FreeChildAreas.Count == 0)
                    {
                        ...
                        if (areaSize.Width == area.Size.Width)
                        {
                            if (areaSize.Height == area.Size.Height)
                                return FillResult.FullFit;

                            AddFreeChildArea(new Rect(area.Position.X, area.Position.Y + areaSize.Height, area.Size.Width, area.Size.Height - areaSize.Height));
                            return FillResult.PartialFit;
                        }
                        else if (areaSize.Height == area.Size.Height)
                        {
                            AddFreeChildArea(new Rect(area.Position.X + areaSize.Width, area.Position.Y, area.Size.Width - areaSize.Width, area.Size.Height));
                            return FillResult.PartialFit;
                        }
                        else
                        {
                            AddFreeChildArea(new Rect(area.Position.X + areaSize.Width, area.Position.Y, area.Size.Width - areaSize.Width, areaSize.Height));
                            AddFreeChildArea(new Rect(area.Position.X, area.Position.Y + areaSize.Height, area.Size.Width, area.Size.Height - areaSize.Height));
                            return FillResult.PartialFit;
                        }
                    }
            ...
            public static KeyValuePair<IGraphic, List<Rect>> PackTextureAtlas(List<IGraphic> graphics, bool indexed)
            {
                if (graphics.Count == 0)
        ...
        public static GraphicAtlas FromGraphics(int paletteIndex, List<Graphic> graphics, bool alpha, int colorIndexOffset = 0)
        {
            if (graphics.Count != 0)
            {
                if (paletteIndex >= 0 && !graphics[0].IndexedGraphic)
                    throw new AmberException(ExceptionScope.Data, "FromGraphics was called with a palette index but the given graphics are not indexed. Use indexed graphics instead.");
                else if (paletteIndex < 0 && graphics[0].IndexedGraphic)
                    throw new AmberException(ExceptionScope.Data, "FromGraphics was called without a palette index but the given graphics are indexed. Use RGBA graphics instead.");
            }
        ...
        public static GraphicAtlas FromTiles(int paletteIndex, List<Graphic> tiles, bool alpha, int colorIndexOffset = 0)
        {
            if (tiles.Select(t => new { t.Width, t.Height, t.IndexedGraphic, t.Data.Length }).Distinct().Count() > 1)
                throw new AmberException(ExceptionScope.Data, "Tile graphics are expected to have all the same size and color depth.");

            if (tiles.Count != 0)
            {
                if (paletteIndex >= 0 && !tiles[0].IndexedGraphic)
                    throw new AmberException(ExceptionScope.Data, "FromTiles was called with a palette index but the given graphics are not indexed. Use indexed graphics instead.");
                else if (paletteIndex < 0 && tiles[0].IndexedGraphic)
                    throw new AmberException(ExceptionScope.Data, "FromTiles was called without a palette index but the given graphics are indexed. Use RGBA graphics instead.");
            }
            ...
            else
            {
                ...
                if (tileWidth == 0 || tileHeight == 0)
                    throw new AmberException(ExceptionScope.Data, "Tile graphics are expected to have a size greater than zero.");
        ...
        public void Read(IDataReader dataReader, uint _, GameData __)
        {
            ...
            if (tiles)
            {
                ...
                if (Texture!.Width % width != 0 || Texture.Height % height != 0)
                    throw new AmberException(ExceptionScope.Data, "Tiled texture atlas dimensions don't fit to the given tile size.");
            ...
            else
            {
                ...
                if (textureAreas.Min(a => a.Left) < 0 ||
                    ...
                    throw new AmberException(ExceptionScope.Data, "Texture atlas entry was outside the atlas boundaries.");
            }
        }
        ...
        public void Write(IDataWriter dataWriter)
        {
            if (textureAreas.Count > ushort.MaxValue)
                throw new AmberException(ExceptionScope.Data, $"Texture atlas areas are limited to {ushort.MaxValue}.");

            if (paletteIndex is null && (flags & 0x20) != 0)
                throw new AmberException(ExceptionScope.Data, "Texture atlas has the 'use palette' flag set but no palette index was given.");

            if (paletteIndex is not null && (flags & 0x20) == 0)
                throw new AmberException(ExceptionScope.Data, "Texture atlas has the 'use palette' flag cleared but a palette index was given.");
            ...
            void WriteRBGAImageSize(int width, int height)
            {
                if (width <= 0 || height <= 0 || width > ushort.MaxValue || height > ushort.MaxValue)
                    throw new AmberException(ExceptionScope.Data, $"Given texture size is out of range. Should be 1x1 to {ushort.MaxValue}x{ushort.MaxValue}.");
            ...
            void WriteAmigaImageSize(int width, int height)
            {
                if (width <= 1 || height <= 1 || (width != 320 && width > 256) || height > 256)
                    throw new AmberException(ExceptionScope.Data, "Given texture size is out of range. Should be 2x2 to 256x256 or exactly 320xH where H is at max 256.");
            ...
            void WriteAtlas()
            {
                if (Texture.Width > ushort.MaxValue || Texture.Height > ushort.MaxValue)
                    throw new AmberException(ExceptionScope.Data, $"Texture atlas size exceeded max value of {ushort.MaxValue}x{ushort.MaxValue}.");

                if (Texture.Width * Texture.Height * (usePalette ? 1 : 4) != Texture.Data.Length)
                    throw new AmberException(ExceptionScope.Data, "Texture atlas data size does not match the excepted size based on the dimensions and format.");
                ...
                if (usePalette && colorIndexOffset != 0)
                {
                    byte AdjustAndCheck(byte colorIndex)
                    {
                        if (colorIndex < colorIndexOffset)
                            throw new AmberException(ExceptionScope.Data, "Invalid color indices with the given color index offset.");
            ...
            else
            {
                ...
                foreach (var area in textureAreas)
                {
                    ...
                    if (area.X < 0 || area.Y < 0 || area.X > ushort.MaxValue || area.Y > ushort.MaxValue ||
                        ...
                        throw new AmberException(ExceptionScope.Data, "The given texture area was outside the texture atlas.");
