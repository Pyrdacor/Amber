    ...
    internal static class PADF
    {
        ...
        public static IFileSpec Read(IDataReader reader, GameData gameData)
        {
            if (!FileHeader.CheckHeader(reader, Header, true))
                throw new AmberException(ExceptionScope.Data, "The file is no PADF");

            string fileType = reader.ReadString(3);

            if (!FileSpecs.TryGetValue(fileType, out var fileSpecProvider))
                throw new AmberException(ExceptionScope.Data, $"Unknown data file in PADF: {fileType}");

            var fileSpec = fileSpecProvider();
            byte fileSpecVersion = reader.ReadByte();

            if (fileSpecVersion > fileSpec.SupportedVersion)
                throw new AmberException(ExceptionScope.Data, $"This application only supports {fileType} versions up to {(int)fileSpec.SupportedVersion} but file has version {(int)fileSpecVersion}");
