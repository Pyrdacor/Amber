 ...
 */
	...
	public static void Write(IDataWriter dataWriter, List<IHunk> hunks)
	{
		...
		foreach (var hunk in realHunks)
		{
			if ((hunk.MemoryFlags & 0x3fffffff) != 0 ||
				...
				throw new AmberException(ExceptionScope.Data, "Invalid hunk memory flags");

			if (hunk.Size > 0x3fffffff)
				throw new AmberException(ExceptionScope.Data, "Invalid hunk size");
			...
			switch (hunk.Type)
				...
				case HunkType.Data:
					...
					if (dataHunk.Data.Length != dataHunk.NumEntries * 4)
						throw new AmberException(ExceptionScope.Data, "Mismatching NumEntries value and Data length for code/data hunk.");

					if (dataHunk.NumEntries != hunk.Size)
						throw new AmberException(ExceptionScope.Data, "Mismatching NumEntries value and hunk size for code/data hunk.");
				...
				case HunkType.BSS:
					...
					if (bssHunk.NumEntries != hunk.Size)
						throw new AmberException(ExceptionScope.Data, "Mismatching NumEntries value and hunk size for BSS hunk.");
				...
				case HunkType.RELOC32:
					...
					if (dataWriter.Position - start != hunk.Size)
						throw new AmberException(ExceptionScope.Application, "Error writing RELOC32 hunk data");
					break;
				}
                }
		}
	...
	public static List<IHunk> Read(IDataReader dataReader, bool deplodeIfNecessary = true)
		...
		static void Throw()
		{
			throw new AmberException(ExceptionScope.Data, "Invalid executable file.");
		}
		...
		for (int i = 0; i < numHunks; ++i)
		{
			...
			switch (type)
			{
				...
				default:
					throw new AmberException(ExceptionScope.Data, $"Unsupported hunk type: {type}.");
			}
			...
			if (flags == 2 || flags == 3) // BSS or DATA
				...
				if (reader.Position < reader.Size && (reader.PeekDword() & 0x3fffffff) != 0) // a size follows -> no BSS but DATA
				{
					...
					if (hunkSize * 4 != hunkSizes[hunkSizeIndex])
						throw new AmberException(ExceptionScope.Data, "Invalid hunk data size.");
	...
	static unsafe byte[] Deplode(List<IHunk> imploderHunks, out List<uint> deplodedHunkSizes, out List<uint> deplodedMemFlags)
		...
		fixed (byte* ptr = &data[0])
		{
			if (!Deploder.Deplode(ptr, buffer, table, dataSize, firstLiteralLength, initialBitBuffer))
				throw new AmberException(ExceptionScope.Data, "Invalid imploded data.");
		}
